...
from pytorch_pretrained_bert.tokenizer import BertTokenizer
...
class BERTDataset(Dataset):
    def __init__(
        ...
        self.corpus_lines = (
            corpus_lines
        )  # number of non-empty lines in input corpus
...
class InputExample(object):
    ...
    def __init__(
        self, guid, tokens_a, tokens_b=None, is_next=None, lm_labels=None
    ):
...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, input_mask, segment_ids, is_next, lm_label_ids
    ):
...
def main():
    ...
    parser.add_argument(
        "--learning_rate",
        default=3e-5,  # 3e-4: Karpathy's learning rate constant for Adam (J4F)
        type=float,
    ...
    tokenizer = BertTokenizer.from_pretrained(
        "models/wordpiece", do_lower_case=args.do_lower_case
    )
